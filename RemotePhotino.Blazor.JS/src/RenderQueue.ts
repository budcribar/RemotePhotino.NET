...
export class RenderQueue {
    ...
    public async processBatch(receivedBatchId: number, batchData: Uint8Array): Promise<void> {
        ...
        if (receivedBatchId > this.nextBatchId) {
            if (this.fatalError) {
                ...
                await DotNet.invokeMethodAsync('Photino.Blazor', 'OnRenderCompleted', this.nextBatchId - 1, this.fatalError.toString());
                return;
            }
            return;
        }
        ...
        try {
            ...
            DotNet.invokeMethodAsync('Photino.Blazor', 'OnRenderCompleted', receivedBatchId, error.toString());
            throw error;
        }
    }
    ...
    private async completeBatch(batchId: number): Promise<void> {
        try {
            await DotNet.invokeMethodAsync('Photino.Blazor', 'OnRenderCompleted', batchId, null);
        } catch {
